package com.mr.webserver.handler;
...
public class DefaultHandler extends
    ...
    public void handle(String target, Request baseRequest,
        ...
        Handler[] handlers = server == null ? null : server
        ...
        for (; handlers != null && i < handlers.length; i++) {
            ...
            if (context.isRunning()) {
                ...
                if (context.getVirtualHosts() != null
                        && context.getVirtualHosts().length > 0)
                    writer.write("http://" + context.getVirtualHosts()[0] + ":"
                            + request.getLocalPort());
                writer.write(context.getContextPath());
                if (context.getContextPath().length() > 1
                        && context.getContextPath().endsWith("/"))
                    writer.write("/");
                writer.write("\">");
                writer.write(context.getContextPath());
                if (context.getVirtualHosts() != null
                        && context.getVirtualHosts().length > 0)
                    writer.write("&nbsp;@&nbsp;" + context.getVirtualHosts()[0]
                            + ":" + request.getLocalPort());
                writer.write("&nbsp;--->&nbsp;");
